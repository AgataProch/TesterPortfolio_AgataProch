...
public final class ApplicationAssertion {
    ...
    // Vytváříme assertaci pro to, že po vyplnění neplatného data narození vyskočí error hláška.
    public void checkBirthDateVerificationError() {
        var errorMessage = elementFinder.findByXPath("/html/body/div/div/div/div/div/form/table/tbody/tr[6]/td[2]/span/strong");
        assertThat(errorMessage.getText()).isEqualTo("Pole musí být platné datum, například 1.1.2000");//Zkontroluj (assertThat) + co zkontroluj (errorMessage), getText (nativní metoda Javy, najde si ten text na stránce podle elementu a zkontroluje text)
    }
}
